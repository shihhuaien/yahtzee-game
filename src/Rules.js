...
class FullHouse extends Rule {
  evalRoll = dice => {
    ...
    const threeOfKind = freqs.includes(3);
    const twoOdKind = freqs.includes(2);
    return threeOfKind && twoOdKind ? this.score : 0
  }
}
...
class SmallStraight extends Rule {
  evalRoll = dice => {
    ...
    if (straight.has(1) & straight.has(2) & straight.has(3) & straight.has(4)) {
      return this.score;
    }
    else if (straight.has(2) & straight.has(3) & straight.has(4) & straight.has(5)) {
      return this.score;
    }
    else if (straight.has(3) & straight.has(4) & straight.has(5) & straight.has(6)) {
      return this.score;
    }
    else {
      return 0
...
const fullHouse = new FullHouse({ score: 25, description: '25 point for a full hause' });

// small/large straights score as 30/40
const smallStraight = new SmallStraight({ score: 30, description: '30 point for a small stright' });
const largeStraight = new LargeStraight({ score: 40, description: '40 point for a large stright' });
